...
import { safePlaqadReturnTo, startPlaqadLogin } from '@/core/auth/PlaqadAuth';
...
const PlaqadLogin = () => {
	...
	const signIn = async () => {
		...
		setError(null);
		try {
			await startPlaqadLogin(safePlaqadReturnTo(location.state?.returnTo));
		} catch (cause) {
		...
		<main className='min-h-screen bg-zinc-950 text-white flex items-center justify-center px-6'>
			<section className='w-full max-w-md rounded-2xl border border-white/10 bg-zinc-900 p-8 shadow-2xl'>
				...
				<div className='mb-8'>
					...
					<h2 className='text-2xl font-semibold tracking-tight'>Operator access</h2>
					<p className='mt-2 text-sm leading-6 text-zinc-400'>
						Continue through Plaqad Account. Your approved operator access and live Super Admin status are checked on every dashboard
						request.
					</p>
				</div>
