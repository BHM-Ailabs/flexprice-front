		...
		<main className='min-h-screen bg-zinc-950 text-white flex items-center justify-center px-6'>
			<section className='w-full max-w-md rounded-2xl border border-white/10 bg-zinc-900 p-8 shadow-2xl'>
				<div className='mb-8 flex items-center gap-3'>
					<img src={flexpriceLogo} alt='FlexPrice' className='h-10 w-10 rounded-lg' />
					<div>
						<p className='text-xs uppercase tracking-[0.18em] text-zinc-400'>Plaqad Control Plane</p>
						<h1 className='text-xl font-semibold'>FlexPrice</h1>
					</div>
				</div>
				...
				<Button onClick={signIn} className='h-11 w-full bg-white text-zinc-950 hover:bg-zinc-200' isLoading={loading}>
					Continue with Plaqad
				</Button>
				<p className='mt-5 text-center text-xs text-zinc-500'>Native FlexPrice credentials cannot authorize this dashboard.</p>
			</section>
		</main>
	);
};
